    ...
    return useMutation({
        mutationFn: async (data: any) => {
            ...
            if (!response.ok) {
                ...
                throw new Error(errData.message || errData.error || 'Failed to create PO');
            }
...
function useUpdatePO(businessId: string | null, user: User | null | undefined) {
    ...
    return useMutation({
        mutationFn: async (data: any) => {
            ...
            if (!response.ok) {
                const errData = await response.json();
                throw new Error(errData.message || errData.error || 'Failed to update PO');
            }
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['purchaseOrders', businessId] });
            toast({ title: 'Purchase Order Updated', description: 'PO has been updated successfully.' });
        },
        onError: (error: Error) => {
            toast({ title: 'Failed to Update PO', description: error.message, variant: 'destructive' });
        },
    });
}
...
function useDeletePO(businessId: string | null, user: User | null | undefined) {
    ...
    return useMutation({
        mutationFn: async (purchaseOrderId: string) => {
            ...
            if (!response.ok) {
                const errData = await response.json();
                throw new Error(errData.message || errData.error || 'Failed to delete PO');
            }
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['purchaseOrders', businessId] });
            toast({ title: 'Purchase Order Deleted', description: `${data.deletedPoNumber} has been deleted.` });
        },
        onError: (error: Error) => {
            toast({ title: 'Failed to Delete PO', description: error.message, variant: 'destructive' });
        },
    });
}
...
function POFormDialog({
    ...
    const handleSubmit = () => {
        onSubmit({
            ...
            supplierPartyId: supplierPartyId.trim(),
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="space-y-3">
                            {items.map((item, index) => (
                                <div
                                    key={index}
                                    className="p-3 border rounded-lg space-y-3 relative bg-muted/20"
                                >
                                    {items.length > 1 && (
                                        <Button
                                            type="button"
                ...
                <div className="space-y-5 py-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        <div className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground">{po.supplierPartyId}</p>
                        </div>
                        <div className="space-y-1">
                            <p className="text-muted-foreground">Warehouse</p>
                            <p className="font-medium">{po.warehouseName || po.warehouseId}</p>
                        </div>
                        <div className="space-y-1">
                        ...
                        {po.completedAt && (
                            <div className="space-y-1">
                                ...
                                <p className="font-medium">{formatDate(po.completedAt)}</p>
                            </div>
                        )}
                        {po.cancelledAt && (
                            <div className="space-y-1">
                                <p className="text-muted-foreground">Cancelled</p>
                                <p className="font-medium">{formatDate(po.cancelledAt)}</p>
                                {po.cancelReason && (
                                    <p className="text-xs text-destructive">{po.cancelReason}</p>
                                )}
                            </div>
                        )}
                    </div>
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        This action will cancel the purchase order. Please provide a reason.
                    </DialogDescription>
                </DialogHeader>
...
export default function PurchaseOrdersPage() {
        ...
        if (searchQuery.trim()) {
            ...
            result = result.filter(
                po =>
                    ...
                    po.orderedSkus.some(sku => sku.toLowerCase().includes(q)) ||
        ...
        result.sort((a, b) => {
            ...
            switch (sortField) {
                ...
                case 'totalAmount':
                    ...
                    break;
                case 'expectedDate':
                    aVal = a.expectedDate?.toDate?.()?.getTime?.() || 0;
                    bVal = b.expectedDate?.toDate?.()?.getTime?.() || 0;
                    break;
                case 'createdAt':
                default:
    ...
    const paginatedPOs = filteredAndSorted.slice(
    ...
    const stats = useMemo(() => {
        const counts: Record<POStatus, number> = {
            draft: 0, confirmed: 0, partially_received: 0,
            fully_received: 0, closed: 0, cancelled: 0,
        };
    ...
    const handleEditSubmit = (data: any) => {
        updateMutation.mutate(data, { onSuccess: () => setEditingPO(null) });
    };

    const handleDelete = () => {
        if (!deletingPO) return;
        deleteMutation.mutate(deletingPO.id, { onSuccess: () => setDeletingPO(null) });
    };

    const handleStatusUpdate = (po: PurchaseOrder, newStatus: POStatus) => {
    ...
    const handleCancelConfirm = (reason: string) => {
        ...
        updateMutation.mutate(
            { purchaseOrderId: cancellingPO.id, status: 'cancelled', cancelReason: reason || null },
            { onSuccess: () => setCancellingPO(null) }
        );
    };
    ...
    return (
        <div className="min-h-full p-6 space-y-6">
                ...
                <CardContent className="p-4">
                    <div className="flex flex-col sm:flex-row gap-3">
                        <div className="relative flex-1">
                            ...
                            <Input
                                placeholder="Search by PO number, supplier, SKU, warehouse..."
                                value={searchQuery}
                        ...
                        >
                            ...
                            <SelectContent>
                                ...
                                <SelectItem value="expectedDate-asc">Expected (Soonest)</SelectItem>
                                <SelectItem value="expectedDate-desc">Expected (Latest)</SelectItem>
                                <SelectItem value="totalAmount-desc">Amount (High → Low)</SelectItem>
                                <SelectItem value="totalAmount-asc">Amount (Low → High)</SelectItem>
                                <SelectItem value="poNumber-asc">PO # (A → Z)</SelectItem>
                                <SelectItem value="poNumber-desc">PO # (Z → A)</SelectItem>
                            </SelectContent>
                        </Select>
                    </div>
                </CardContent>
                ...
                {isLoading ? (
                    ...
                    <CardContent className="p-12 text-center">
                        ...
                        <h3 className="text-lg font-semibold mb-1">
                            {purchaseOrders.length === 0 ? 'No Purchase Orders Yet' : 'No Matching Results'}
                        </h3>
                                            ...
                                            <TableCell onClick={(e) => e.stopPropagation()}>
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        ...
                                                        {po.status === 'draft' && (
                                                            <DropdownMenuItem onClick={() => handleStatusUpdate(po, 'confirmed')}>
                                                        ...
                                                        {po.status === 'fully_received' && (
                                                            <DropdownMenuItem onClick={() => handleStatusUpdate(po, 'closed')}>
            ...
            <PODetailDialog
                open={!!viewingPO}
                onOpenChange={(open) => { if (!open) setViewingPO(null); }}
                po={viewingPO}
            />

            <CancelPODialog
                open={!!cancellingPO}
                onOpenChange={(open) => { if (!open) setCancellingPO(null); }}
                onConfirm={handleCancelConfirm}
                isLoading={updateMutation.isPending}
                poNumber={cancellingPO?.poNumber || ''}
            />
            ...
            <AlertDialog open={!!deletingPO} onOpenChange={(open) => { if (!open) setDeletingPO(null); }}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Delete {deletingPO?.poNumber}?</AlertDialogTitle>
                        <AlertDialogDescription>
