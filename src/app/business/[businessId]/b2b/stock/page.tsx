...
function StockDialog({ open, onClose, material, businessId, user, adjust = false }: StockDialogProps) {
    ...
    const handleSubmit = async () => {
        ...
        if (!adjust && qty <= 0) { toast({ title: 'Quantity must be positive', variant: 'destructive' }); return; }
        if (!adjust && !referenceId.trim()) { toast({ title: 'Reference ID required', variant: 'destructive' }); return; }
        if (adjust && !note.trim()) { toast({ title: 'Note is required for adjustments', variant: 'destructive' }); return; }
        ...
        try {
            ...
            const payload: Record<string, unknown> = {
                businessId, materialId: material.id, quantity: qty,
        ...
        <Dialog open={open} onOpenChange={o => !o && onClose()}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {material?.name} · Available: <span className="font-mono font-medium">{material?.availableStock} {material?.unit}</span>
                    </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 py-2">
                    <div className="space-y-2">
                        ...
                        <Input type="number" placeholder={adjust ? 'e.g. -50 or +100' : 'e.g. 500'}
                            value={quantity} onChange={e => setQuantity(e.target.value)} step="0.01" />
                    </div>
                    {!adjust && (
                        <div className="space-y-2">
                            <Label className="text-xs">Reference ID / PO Number <span className="text-destructive">*</span></Label>
                            <Input placeholder="e.g. PO-2024-001" value={referenceId} onChange={e => setRef(e.target.value)} />
                        </div>
                    )}
                    <div className="space-y-2">
                        <Label className="text-xs">Note {adjust && <span className="text-destructive">*</span>}</Label>
                        <Input placeholder={adjust ? 'Reason for adjustment (required)' : 'Optional note'}
...
export default function StockPage() {
    ...
    const handleCreate = async () => {
        ...
        if (isNaN(level) || level < 0) { toast({ title: 'Invalid reorder level', variant: 'destructive' }); return; }
        setIsCreating(true);
        try {
            const token = await user.getIdToken();
            const res = await fetch('/api/business/b2b/create-raw-material', {
                ...
                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                body: JSON.stringify({
                    businessId, name: name.trim(), sku: sku.trim().toUpperCase(),
                    unit: unit.trim(), category: category.trim(), reorderLevel: level,
    ...
    return (
        <div className="flex flex-col h-full">
            <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }}
                ...
                {/* Stats — no "reserved" concept anymore */}
                <div className="grid grid-cols-2 gap-3 mb-4">
                    {[
                        { label: 'Low Stock', value: lowStockCount, color: 'text-amber-600', bg: 'bg-amber-50 border-amber-200 dark:bg-amber-950/20 dark:border-amber-700' },
                        { label: 'Out of Stock', value: outOfStockCount, color: 'text-red-600', bg: 'bg-red-50 border-red-200 dark:bg-red-950/20 dark:border-red-700' },
                    ].map(stat => (
                ...
                <div className="relative max-w-sm">
                    ...
                    <Input placeholder="Search by name, SKU, category..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 h-9" />
            ...
            <div className="flex-1 overflow-auto">
                {loading ? (
                    <div className="p-6 space-y-3">
                        {Array.from({ length: 6 }).map((_, i) => <div key={i} className="flex items-center gap-4"><Skeleton className="h-5 w-32" /><Skeleton className="h-5 w-20" /><Skeleton className="h-5 w-48 ml-auto" /></div>)}
                    </div>
                ) : filtered.length === 0 ? (
                    ...
                    <Table>
                            ...
                            <TableRow>
                                ...
                                <TableHead className="text-right">
                                    <Button variant="ghost" size="sm" className="h-8 gap-1 font-semibold" onClick={() => handleSort('totalStock')}>
                                        Total Stock <ArrowUpDown className="h-3 w-3" />
                                    </Button>
                                </TableHead>
                                <TableHead className="text-right">
                        ...
                        <TableBody>
                            <AnimatePresence mode="popLayout">
                                {filtered.map((mat, i) => {
                                    const pct = mat.totalStock > 0 ? Math.round((mat.availableStock / mat.totalStock) * 100) : 0;
                                    const isLow = mat.availableStock > 0 && mat.availableStock <= mat.reorderLevel;
                                    const isOut = mat.availableStock <= 0;
                                    return (
                                        <motion.tr key={mat.id} custom={i} variants={rowVariants} initial="hidden" animate="visible" exit={{ opacity: 0 }} layout
                                            className="group border-b hover:bg-muted/40 transition-colors cursor-pointer"
                                            onClick={() => router.push(`/business/${businessId}/b2b/stock/${mat.id}`)}>
                                            <TableCell>
                                                <div className="flex items-center gap-2">
                                                    {(isOut || isLow) && <AlertTriangle className={cn('h-3.5 w-3.5 shrink-0', isOut ? 'text-red-500' : 'text-amber-500')} />}
                                                    <div>
                                            ...
                                            <TableCell><Badge variant="secondary" className="text-xs font-normal">{mat.category}</Badge></TableCell>
                                            ...
                                            <TableCell className="text-right">
                                                <span className={cn('font-mono text-sm font-semibold', stockHealthColor(mat))}>
                                                    {mat.availableStock.toLocaleString()} <span className="text-xs font-normal">{mat.unit}</span>
                                                </span>
                                            </TableCell>
                                            <TableCell>
                                                <div className="flex items-center gap-2">
                                                    <div className="h-2 flex-1 rounded-full bg-muted overflow-hidden">
                                                        <div className={cn('h-full rounded-full transition-all', stockHealthBg(mat))} style={{ width: `${Math.max(0, Math.min(100, pct))}%` }} />
                                                    </div>
                                            ...
                                            <TableCell onClick={e => e.stopPropagation()}>
                                                <div className="flex items-center gap-1">
                                                    <Button variant="outline" size="sm" className="h-7 text-xs px-2 gap-1" onClick={() => setAddStockTarget(mat)}>
                                                        <TrendingUp className="h-3 w-3" /> Add
                                                    </Button>
                                                    <Button variant="ghost" size="sm" className="h-7 text-xs px-2 gap-1" onClick={() => setAdjustStockTarget(mat)}>
            ...
            <Dialog open={createOpen} onOpenChange={o => !o && setCreateOpen(false)}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2"><PackagePlus className="h-5 w-5 text-primary" />Add Raw Material</DialogTitle>
                    ...
                    <div className="grid grid-cols-2 gap-4 py-2">
                        <div className="col-span-2 space-y-2">
                            ...
                            <Input placeholder="e.g. White Cotton Fabric" value={createForm.name} onChange={e => setCreateForm(f => ({ ...f, name: e.target.value }))} />
                        </div>
                        <div className="space-y-2">
                            <Label className="text-xs">SKU <span className="text-destructive">*</span></Label>
                            <Input placeholder="e.g. FAB-WHT-001" className="font-mono" value={createForm.sku} onChange={e => setCreateForm(f => ({ ...f, sku: e.target.value.toUpperCase() }))} />
                        </div>
                        <div className="space-y-2">
                            <Label className="text-xs">Unit <span className="text-destructive">*</span></Label>
                            <Input placeholder="e.g. metres, kg, pieces" value={createForm.unit} onChange={e => setCreateForm(f => ({ ...f, unit: e.target.value }))} />
                        </div>
                        <div className="space-y-2">
                            <Label className="text-xs">Category <span className="text-destructive">*</span></Label>
                            <Input placeholder="e.g. Fabric, Thread, Trim" value={createForm.category} onChange={e => setCreateForm(f => ({ ...f, category: e.target.value }))} />
                        </div>
                        <div className="space-y-2">
                            <Label className="text-xs">Reorder Level <span className="text-destructive">*</span></Label>
                            <Input type="number" min="0" placeholder="e.g. 100" value={createForm.reorderLevel} onChange={e => setCreateForm(f => ({ ...f, reorderLevel: e.target.value }))} />
                        </div>
                        <div className="col-span-2 space-y-2">
                            <Label className="text-xs">Supplier Name</Label>
                            <Input placeholder="Optional" value={createForm.supplierName} onChange={e => setCreateForm(f => ({ ...f, supplierName: e.target.value }))} />
                        </div>
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setCreateOpen(false)}>Cancel</Button>
                        <Button onClick={handleCreate} disabled={isCreating} className="gap-2">
                            {isCreating && <Loader2 className="h-4 w-4 animate-spin" />} Add Material
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>

            <StockDialog open={!!addStockTarget} onClose={() => setAddStockTarget(null)} material={addStockTarget} businessId={businessId} user={user} adjust={false} />
            <StockDialog open={!!adjustStockTarget} onClose={() => setAdjustStockTarget(null)} material={adjustStockTarget} businessId={businessId} user={user} adjust={true} />
        </div>
    );
}