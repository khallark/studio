...
export default function PickupLocationsPage() {
    ...
    useEffect(() => {
        if (storeId && memberRole && user) {
            setLoading(true);

            let locationsRef;
            if (memberRole === 'Vendor') {
                locationsRef = collection(db, 'accounts', storeId, 'members', user.uid, 'pickupLocations');
            } else {
                locationsRef = collection(db, 'accounts', storeId, 'pickupLocations');
            }
            ...
            return () => unsubscribe();
        } else if (!authLoading && (!storeId || !memberRole)) {
            setLoading(false);
        }
    }, [storeId, memberRole, user, toast, authLoading]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const body = isEditing
            ? JSON.stringify({ shop: storeId, locationId: editingLocation.id, location: locationData })
            : JSON.stringify({ shop: storeId, location: locationData });
    ...
    const handleDeleteLocation = async (locationId: string) => {
        ...
        try {
            ...
            const response = await fetch('/api/shopify/locations/delete', {
                ...
                body: JSON.stringify({ shop: storeId, locationId }),
            });
            const result = await response.json();
            if (!response.ok) {
            ...
            <Card className="w-full max-w-4xl">
                <CardHeader>
                    <div className="flex items-center justify-between">
                        ...
                        {memberRole !== 'Staff' && (
                            <Button disabled={!storeId} onClick={() => handleOpenDialog()}>
                        ...
                        <div className="flex flex-1 items-center justify-center rounded-lg border border-dashed shadow-sm min-h-[300px]">
                            <div className="flex flex-col items-center gap-1 text-center">
                                ...
                                <p className="text-sm text-muted-foreground">
                                    {storeId ? 'Click "Add New Location" to get started.' : 'Please connect a store first.'}
                                </p>
                            </div>
                        </div>
                    ) : (
