// /api/business/generate-blue-dart-remittance-table/route.ts
    ...
    try {
        ...
        await fetch(CLOUD_FUNCTION_URL, {
            ...
            businessDocRef.update({
                ...
                'blueDartRemittanceTable.error': err.message ?? 'Failed to call cloud function',
            });
        });

        return NextResponse.json({ success: true, status: 'processing' }, { status: 202 });
    } catch (error: unknown) {
        console.error('[generate-blue-dart-remittance-table] Error:', error);
