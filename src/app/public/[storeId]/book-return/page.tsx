...
const STATUS_MAP: Record<string, string> = {
  ...
  'Lost': 'Lost',
  'Closed': 'Delivered',
  'RTO Processed': 'Return Completed',
...
export default function BookReturnPage() {
  ...
  const notEligible = ['New', 'Confirmed', 'Ready To Dispatch', 'Dispatched', 'RTO Delivered', 'Lost', 'Closed', 'RTO Processed', 'RTO Closed', 'Cancellation Requested'];
  const alreadyInProcess = ['DTO Booked', 'DTO In Transit', 'DTO Delivered', 'Pending Refunds', 'DTO Refunded'];
            ...
            <CardContent className="space-y-6">
              {returnResponse ? (
                ...
                <>
                  <div className="bg-muted/50 border rounded-lg p-4">
                    <p className="text-xs sm:text-sm font-bold leading-relaxed">
                      {(() => {
                        ...
                        if (order.status === 'DTO Refunded') {
                          return (
                            <>
                              <span className="text-gray-600">Refund Pending</span>
                              <br />
                              Your return has been successfully refunded either in your store wallet or the bank account. If you have any questions, please DM us on Instagram @owr.life.
                            </>
                          );
                        }
                        if (canTryRequesting.includes(order.status)) {
                  ...
                  {!(notEligible.includes(order.status) || alreadyInProcess.includes(order.status)) && <><div>
