// app/api/business/generate-cod-report/route.ts
import { NextRequest, NextResponse } from 'next/server';
import { authUserForBusiness } from '@/lib/authoriseUser';

const CLOUD_FUNCTION_URL = process.env.GENERATE_COD_REPORT_URL!;
const ENQUEUE_FUNCTION_SECRET = process.env.ENQUEUE_FUNCTION_SECRET!;

// Couriers the COD report supports for now.
const SUPPORTED_COURIERS = ['Blue Dart', 'Delhivery'] as const;
type SupportedCourier = (typeof SUPPORTED_COURIERS)[number];

export async function POST(req: NextRequest) {
    try {
        const { businessId, courier, startDate, endDate } = await req.json();
        ...
        // Validate courier (stores are resolved server-side from businessDoc.stores
        // by the Cloud Function, so no storeIds are accepted here).
        if (!courier || !SUPPORTED_COURIERS.includes(courier as SupportedCourier)) {
            return NextResponse.json(
                {
                    error: 'Validation Error',
                    message: `courier is required and must be one of: ${SUPPORTED_COURIERS.join(', ')}`,
                },
                { status: 400 }
            );
        }
        ...
        console.log('🚀 Calling COD Cloud Function:', {
        ...
        console.log('✅ COD Cloud Function call successful:', {
        ...
        return NextResponse.json(
            {
                success: true,
                message: 'COD report generation has been queued.',
                taskName: responseData.taskName,
        ...
        console.error('❌ COD report API error:', error);
        return NextResponse.json(
            {
